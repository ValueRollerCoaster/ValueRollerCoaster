...
Company Profile (from Company Context Manager)
    ↓
Value Components (from Database)
    ↓
...
Value Alignment Workflow
    ├── Profiler Agent (identify needs)
    ├── Hypothesizer Agent (select relevant values)
    └── Final Aligner (create alignment matrix)
    ↓
...
1. **`value_components`**
   - Stores company value propositions
   - Vector dimension: 1536
   - Used for: Value alignment matching
...
```python
{
    ...
    "original_value": str,     # User input
    "ai_processed_value": str, # AI-generated customer benefit
    "user_id": str,           # User identifier (for multi-user)
